...
void CHexEdit::SetCurrPos(int nPosition, int nSelAction, int bInByte)
{
	...
	if (m_bTrailingCaret)
	{
		if ((m_nPosition - 1) < m_nTopPosition)
			SetScroll(m_nPosition - 1);
		else if (m_nPosition > (m_nTopPosition + (GetClientRows() * m_nBytesPerRow)))
			SetScroll(m_nPosition - (GetClientRows() * m_nBytesPerRow));
	}
	else
	{
		if (m_nPosition < m_nTopPosition)
			SetScroll(m_nPosition);
		else if (m_nPosition >= (m_nTopPosition + (GetClientRows() * m_nBytesPerRow)))
			SetScroll(m_nPosition - ((GetClientRows() - 1) * m_nBytesPerRow));
	}
...
void CHexEdit::SetScroll(int nTopPosition)
{
	// Keep position within valid range
	if (nTopPosition < 0)
		nTopPosition = 0;
...
void CHexEdit::UpdateScrollbar()
{
	...
	// Vertical scrollbar
	si.nMin = 0;
	si.nMax = (m_nLength / m_nBytesPerRow);
	si.nPage = GetClientRows();
	si.nPos = (m_nTopPosition / m_nBytesPerRow);
	...
	switch (nSBCode)
	{
		case SB_LINEDOWN:
			if ((m_nTopPosition + m_nBytesPerRow) < m_nLength)
				SetScroll(m_nTopPosition + m_nBytesPerRow);
			break;
		...
		case SB_PAGEDOWN:
			if ((m_nTopPosition + m_nBytesPerRow) < m_nLength)
				SetScroll(m_nTopPosition + (GetClientRows() * m_nBytesPerRow));
			break;
...
void CHexEdit::OnEditCopy()
{
	if (HasSelection())
	{
		COleDataSource* pDataSource = CreateDataSource();
		if (pDataSource != NULL)
...
COleDataSource* CHexEdit::CreateDataSource()
{
...
BOOL CHexEdit::DoPasteData(COleDataObject* pDataObject)
{
	HGLOBAL hMem = NULL;
	BYTE* lpByte;
	int nLength = 0;
